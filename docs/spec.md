...
複数の `.env.*` ファイルを丸ごと切り替えるのではなく、`envs.yml` を設定のソースとして扱う。これにより、環境や状況ごとの値を 1 つのツリーで統合的に確認できるようにする。
...
- source: 設定のソースとなる YAML ファイル。初期値は `envs.yml`。
- target: 実際にアプリケーションが読む dotenv ファイル。初期値は `.env`。
- path: `envs.yml` 内のノードを指すドット区切りのパス。例: `network.home`。
- node: `envs.yml` 内の任意のマッピング。
- var: 文字列テンプレートから参照できる変数を置くための予約キー。
- set: target に反映する key-value を置くための予約キー。
- del: target から無効化する key を置くための予約キー。
...
`set` は予約キーであり、そのノードを target に反映するときの dotenv key-value を表す。
...
- `set` は YAML mapping でなければならない。
- `set` のキーは dotenv のキー名として扱う。
- `set` の値は YAML scalar だけを認める。
- `set` 自体は path の一部として扱わない。
- scalar は YAML 上の文字列表現として扱う。例: `8080` は `8080`、`true` は `true` として扱う。
- mapping や sequence が含まれている場合は失敗し、target は変更しない。

## `del` 規約

`del` は予約キーであり、そのノードを target に反映するときに無効化する dotenv key を表す。
...
`dotenv-switch push.off` を実行すると、target 内の有効な `PUSH_ENABLED=...` 定義行が無効化される。

仕様:

- `del` は YAML sequence でなければならない。
- `del` の各要素は dotenv のキー名として扱う。
- `del` 自体は path の一部として扱わない。
- `del` は空の値を設定する操作ではなく、定義行を無効化する操作である。
- 文字列として扱えない値が含まれている場合は失敗し、target は変更しない。

## `var` 規約

`var` は予約キーであり、`var` と `set` の文字列から参照できる変数を定義する。
...
- path が指す定義ノード内の `var` は、そのノードの `set` から参照できる。
- 定義ノード内の `var` は top-level の `var` を上書きできる。
- `var` 自体は path の一部として扱わない。

## 式展開

`var` の右辺と `set` の右辺では、`${{ }}` で式を開ける。
...
指定した path の `set` と `del` を target に反映する。
複数 path を指定した場合は、指定順に target へ重ねて反映する。
...
- ノードに `set` と `del` のどちらもない場合は失敗する。
- `var` と `set` の式を解決する。
- `del` に含まれる各キーについて、target 内の有効な既存定義を無効化する。
- `set` に含まれる各キーについて、target 内の既存定義を探す。
...
- `envs.yml` 内を再帰的に探索する。
- `set` または `del` を持つノードを path として表示する。
- `var`、`set`、`del` 自体は表示しない。
- `set` と `del` のどちらも持たない中間ノードは表示しない。
- 表示順は YAML 上の出現順とする。
- 辞書順ソートはしない。

### `dotenv-switch show <path>`

指定した path の変更内容を表示する。
...
- `.env` は変更しない。
- `var` と `set` の式を解決した結果を dotenv 形式で表示する。
- `del` に含まれる削除操作は `-KEY` 形式で表示する。
- 初期版に含める。
...
- `del` に含まれる key は、有効な `KEY=value` 形式の行を無効化する。
- その key に対応する `# KEY=...` 形式のコメントアウト行が target 内のどこかに既に存在する場合、有効な `KEY=value` 行は削除する。
- その key に対応する `# KEY=...` 形式のコメントアウト行が存在しない場合、有効な `KEY=value` 行は `# KEY=value` としてその場でコメントアウトする。
- target に同じキーの定義行が複数ある場合は、最後の定義行を更新する。
- target に対象キーの定義行が存在しない場合は、新しい定義行を追加する。
- 追加時は、まず `# KEY=...` 形式のコメントアウト行を探す。
- `# KEY=...` 形式のコメントアウト行が見つかった場合、その最後の行を `KEY=value` に置き換える。
- 同じ key の `# KEY=...` 形式のコメントアウト行が複数ある場合、最後の行だけを定義行にし、それ以外は削除する。
...
`set` の文字列は、式展開後に次の規則で `.env` へ書き出す。
...
- 指定 path に `set` と `del` のどちらもない場合は target を変更しない。
- `var`、`set`、`del` が不正な形式の場合は target を変更しない。
...
- target 内に存在する `KEY=value` 行の無効化。
- target 内に存在しないキーの追加。
- `# KEY=...` コメントアウト行を使ったコメントイン位置の決定。
