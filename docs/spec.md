...
- del: target から削除する key を置くための予約キー。
...
`del` は予約キーであり、そのノードを target に反映するときに削除する dotenv key を表す。
...
`dotenv-switch push.off` を実行すると、target 内の有効な `PUSH_ENABLED=...` 定義行が削除される。
...
- `del` は空の値を設定する操作ではなく、定義行を削除する操作である。
- 文字列として扱えない値が含まれている場合は失敗し、target は変更しない。

## `var` 規約

`var` は予約キーであり、`var` と `out` の文字列から参照できる変数を定義する。
...
指定した path の `out` と `del` を target に反映する。
...
- ノードに `out` と `del` のどちらもない場合は失敗する。
- `var` と `out` の式を解決する。
- `del` に含まれる各キーについて、target 内の有効な既存定義を削除する。
...
- `envs.yml` 内を再帰的に探索する。
- `out` または `del` を持つノードを path として表示する。
- `var`、`out`、`del` 自体は表示しない。
- `out` と `del` のどちらも持たない中間ノードは表示しない。
- 表示順は YAML 上の出現順とする。
- 辞書順ソートはしない。
...
- `.env` は変更しない。
- `var` と `out` の式を解決した結果を dotenv 形式で表示する。
...
- `del` に含まれる key は、有効な `KEY=value` 形式の行を削除する。
...
- コメント行の中にある `KEY=value` は更新対象にも削除対象にもしない。
...
- 指定 path に `out` と `del` のどちらもない場合は target を変更しない。
- `var`、`out`、`del` が不正な形式の場合は target を変更しない。
